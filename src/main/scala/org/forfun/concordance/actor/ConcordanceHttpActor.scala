...
class ConcordanceHttpActor extends Actor with ActorLogging {
  ...
  def receive = {
     ...
     */
    case r@HttpRequest(POST, Uri.Path("/concordance"), headers, entity: HttpEntity.NonEmpty, protocol) =>
      // emulate chunked behavior for POST requests to this path
      val parts = r.asPartStream()
      val client = sender
      val handler = context.actorOf(Props(new ConcordanceHandler(client, parts.head.asInstanceOf[ChunkedRequestStart])))
     ...
     * stream processing..
     */
    case start@ChunkedRequestStart(HttpRequest(POST, Uri.Path("/concordance"), _, _, _)) =>
      val client = sender
      val handler = context actorOf ConcordanceHandler.props(client, start)
